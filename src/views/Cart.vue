<template>
  <div class="container">
    ...
    <div v-if="cartDetail.length===0"
         class="empty">
      <img src="../assets/img/cart-empty.webp"
           alt="">
      <div>去添加点什么吧</div>
    </div>
    <div v-else>
      <div v-for="(item,index) in cartDetail"
           :key="index"
           class="item">
        <input type="checkbox"
        ...
        <div class="detail">
          ...
          <div class="info">
            ...
            <div class="selnum">
              <span class="label less"
                    @click="less(item)">-</span>
              <input type="text"
                     class="label num"
                     v-model.number="item.num"
                     @change="updateCart(item)">
              <span class="label more"
                    @click="more(item)">+</span>
            </div>
          </div>
        </div>
        <i class="iconfont icon-shanchu1"
           @click="delCart(item,index)"></i>
      </div>
    </div>
    <div class="footer">
      <div class="checkball">
      ...
      <div class="confirm"
           @click="confirm"
           :class="{'btn-disable': checkNum===0}">下单</div>
    </div>
    <nav-footer />
